...
const AdviserCard = ({ title, companyName, description, link, index, mediaPath, onClick }) => {
    ...
    return (
        <div className={`${bgClass} min-h-[300px] flex flex-col md:flex-row gap-x-10 gap-y-5 text-white rounded-4xl items-center cursor-pointer`} onClick={onClick} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { onClick?.(); } }}>
            ...
            <div className='w-full md:w-2/5 flex justify-center md:justify-end md:ml-auto'>
                <img
                    src={processImageUrl("/images/advertise.png")}
                    alt=""
