...
export default function ProductModal({ open, setOpen, productId, restaurantData }) {
    ...
    const { data: productData, isLoading, error } = useProductWithAddons(productId, {
    ...
    const isAddToCartDisabled = useMemo(() => {
        ...
        return product.product_addon_categories.some(category => {
            ...
            const selectedCount = categoryAddons.filter(addon => addon.selected).length;
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="max-w-[880px]! p-0 block overflow-y-auto max-h-[90vh]">
                <img src={product.image_url || "/images/product-1.png"} alt="product-modal-image" className="w-full h-[286px] object-cover block" />
                <div className="border-b border-primary-1007 mx-8 py-6 mb-4">
                    ...
                    <p className="text-2xl font-bold text-primary-50">${totalPrice.toFixed(2)}</p>
                </div>
                ...
                {product.product_addon_categories?.map((category) => (
                    <div key={category.id} className="mx-8 mb-6">
                        ...
                        <div className="border border-primary-1007 rounded-xl p-7 mt-4">
                            {category.type === 'radio' ? (
                                <RadioGroup 
                                    value={Object.keys(selectedAddons).find(id => 
                                        selectedAddons[id].categoryId === category.id && selectedAddons[id].selected
                                    ) || ""}
                                    onValueChange={(value) => {
                                ...
                                <div className="space-y-5">
                                    {category.product_addons.map((addon) => (
                                        <div key={addon.id} className="flex items-center space-x-2">
                                            ...
                                            />
                                            <Label htmlFor={addon.id.toString()} className="flex-1">{addon.name}</Label>
                                            {selectedAddons[addon.id]?.selected && (
                                                <div className="flex items-center space-x-2">
                                                    <button 
                                                        onClick={() => handleAddonQuantityChange(addon.id, (selectedAddons[addon.id]?.quantity || 1) - 1)}
                                                    ...
                                                    <span className="font-bold w-8 text-center">{selectedAddons[addon.id]?.quantity || 1}</span>
                                                    <button 
                                                        onClick={() => handleAddonQuantityChange(addon.id, (selectedAddons[addon.id]?.quantity || 1) + 1)}
                                                        className="border-2 border-primary-50 text-primary-50 w-6 h-6 flex justify-center items-center rounded-full cursor-pointer"
                                                    >
